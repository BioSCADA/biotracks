  ...
  public static String[] getFrequencyOptions(Context context, boolean metricUnits) {
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      if (value == PreferencesUtils.FREQUENCY_OFF) {
        ...
        if (metricUnits) {
          options[i] = context.getString(R.string.value_integer_meter, Math.abs(value));
        } else {
          options[i] = context.getString(R.string.value_float_mile, Math.abs(value/1000.0d));
        }
      } else if (value >= 60) {
        options[i] = context.getString(R.string.value_integer_minute, value/60);
      } else {
        options[i] = context.getString(R.string.value_integer_second, value);
      }
    }
    return options;
  }
