...
public class PeriodicTaskExecutor {
  ...
  public void restore() {
    ...
    if (isTimeFrequency()) {
      ...
      timerTaskExecutor.scheduleTask(taskFrequency * SECOND_TO_MILLISECONDS);
    } else {
   ...
   * 
   * @param strings the task parameters
   */
  public void setTaskParams(String[] strings) {
    this.params = strings;
    if (this.periodicTask != null)
      this.periodicTask.setParams(strings);
  }
