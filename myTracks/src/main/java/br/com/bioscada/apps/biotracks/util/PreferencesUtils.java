...
public class PreferencesUtils {
    ...
    public static final String[] VOICE_ANNOUNCEMENT_DEFAULT = new String[0];
    ...
    /**
     * Gets a list of strings preference value.
     *
     * @param context the context
     * @param keyId the key id
     * @param defaultValue default value
     */
    public static String[] getStringList(Context context, int keyId, String[] defaultValue) {
        SharedPreferences sharedPreferences = context.getSharedPreferences(
                Constants.SETTINGS_NAME, Context.MODE_PRIVATE);
        String def = TextUtils.join(DEFAULT_SEPARATOR, defaultValue);
        String prefs = sharedPreferences.getString(getKey(context, keyId), def);
        if (prefs.equals("")) {
            return new String[0];
        } else {
            return prefs.split(DEFAULT_SEPARATOR);
        }
    }

    /**
     * Sets a list of string preference value.
     *
     * @param context the context
     * @param keyId the key id
     * @param value the value
     */
    @SuppressLint("CommitPrefEdits")
    public static void setStringList(Context context, int keyId, String[] values) {
        SharedPreferences sharedPreferences = context.getSharedPreferences(
                Constants.SETTINGS_NAME, Context.MODE_PRIVATE);
        Editor editor = sharedPreferences.edit();
        editor.putString(getKey(context, keyId), TextUtils.join(DEFAULT_SEPARATOR, values));
        ApiAdapterFactory.getApiAdapter().applyPreferenceChanges(editor);
    }
