...
package br.com.bioscada.apps.biotracks.services.sensors;

import android.content.Context;

import br.com.bioscada.apps.biotracks.R;
import br.com.bioscada.apps.biotracks.services.sensors.ant.AntSensorManager;
import br.com.bioscada.apps.biotracks.services.sensors.bluetooth.NeuroskySensorManager;
import br.com.bioscada.apps.biotracks.services.sensors.bluetooth.PolarSensorManager;
import br.com.bioscada.apps.biotracks.services.sensors.bluetooth.ZephyrSensorManager;
import br.com.bioscada.apps.biotracks.services.sensors.btle.BluetoothLeManager;
import br.com.bioscada.apps.biotracks.services.sensors.btle.LeCscSensor;
import br.com.bioscada.apps.biotracks.services.sensors.btle.LeHrmSensor;
...
public class SensorManagerFactory {
  ...
  private static SensorManager getSensorManager(Context context, boolean sendPageViews) {
    ...
    } else if (sensorType.equals(context.getString(R.string.sensor_type_value_btle))) {
      if (sendPageViews) {
        AnalyticsUtils.sendPageViews(context, AnalyticsUtils.SENSOR_BTLE);
      }
      //We only support one device for now, try HRM first
      String addr;
      addr = PreferencesUtils.getString(
          context, R.string.btle_hrm_sensor_id_key, PreferencesUtils.BLUETOOTH_SENSOR_DEFAULT);
      
      if (addr != null && !addr.trim().equals(""))
      {
        return new BluetoothLeManager(context, addr, LeHrmSensor.INSTANCE);
      }
      //Next try CSC
      addr = PreferencesUtils.getString(
          context, R.string.btle_csc_sensor_id_key, PreferencesUtils.BLUETOOTH_SENSOR_DEFAULT);
      
      if (addr != null && !addr.trim().equals(""))
      {
        return new BluetoothLeManager(context, addr, LeCscSensor.INSTANCE);
      }
      return null;
    }
    return null;
