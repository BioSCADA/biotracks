...
package br.com.bioscada.apps.biotracks.services;
...
import br.com.bioscada.apps.biotracks.content.DescriptionGeneratorImpl;
...
import br.com.bioscada.apps.biotracks.services.tasks.AnnouncementPeriodicTaskFactory;
import br.com.bioscada.apps.biotracks.services.tasks.PeriodicTaskExecutor;
import br.com.bioscada.apps.biotracks.services.tasks.SplitPeriodicTaskFactory;
import br.com.bioscada.apps.biotracks.stats.TripStatisticsUpdater;
...
import br.com.bioscada.apps.biotracks.util.TrackNameUtils;
...
public class TrackRecordingService extends Service {
    ...
    public static final String
            RESUME_TRACK_EXTRA_NAME = "br.com.bioscada.apps.biotracks.RESUME_TRACK";
    ...
    private final OnSharedPreferenceChangeListener
        ...
        public void onSharedPreferenceChanged(SharedPreferences preferences, String key) {
            ...
            if (key == null
                ...
                voiceSplitExecutor.setMetricUnits(metricUnits);
                voice1Executor.setMetricUnits(metricUnits);
                voice2Executor.setMetricUnits(metricUnits);
            }
            if (key == null
                    || key.equals(PreferencesUtils.getKey(context, R.string.voice_split_frequency_key))) {
                splitExecutor.setTaskFrequency(PreferencesUtils.getInt(
                        context, R.string.voice_split_frequency_key, PreferencesUtils.SPLIT_FREQUENCY_DEFAULT));
                voiceSplitExecutor.setTaskFrequency(PreferencesUtils.getInt(
                        context, R.string.voice_split_frequency_key, PreferencesUtils.SPLIT_FREQUENCY_DEFAULT));
            }
            if (key == null
                    || key.equals(PreferencesUtils.getKey(context, R.string.voice_1_announcement_key))) {
                voiceSplitExecutor.setTaskParams(PreferencesUtils.getStringList(
                        context, R.string.voice_split_announcement_key, PreferencesUtils.VOICE_ANNOUNCEMENT_DEFAULT));
            }
            if (key == null
                    || key.equals(PreferencesUtils.getKey(context, R.string.voice_1_frequency_key))) {
                voice1Executor.setTaskFrequency(PreferencesUtils.getInt(
                        context, R.string.voice_1_frequency_key, PreferencesUtils.VOICE_FREQUENCY_DEFAULT));
            }
            if (key == null
                    || key.equals(PreferencesUtils.getKey(context, R.string.voice_1_announcement_key))) {
                voice1Executor.setTaskParams(PreferencesUtils.getStringList(
                        context, R.string.voice_1_announcement_key, PreferencesUtils.VOICE_ANNOUNCEMENT_DEFAULT));
            }
            if (key == null
                    || key.equals(PreferencesUtils.getKey(context, R.string.voice_2_frequency_key))) {
                voice2Executor.setTaskFrequency(PreferencesUtils.getInt(
                        context, R.string.voice_2_frequency_key, PreferencesUtils.VOICE_FREQUENCY_DEFAULT));
            }
            if (key == null
                    || key.equals(PreferencesUtils.getKey(context, R.string.voice_2_announcement_key))) {
                voice2Executor.setTaskParams(PreferencesUtils.getStringList(
                        context, R.string.voice_2_announcement_key, PreferencesUtils.VOICE_ANNOUNCEMENT_DEFAULT));
            }
            if (key == null || key.equals(
    ...
    /**
     * Gets the last split waypoint
     */
    public Waypoint getLastSplitWaypoint() {
        return lastSplitWaypoint;
    }
    ...
    public long insertWaypoint(WaypointCreationRequest waypointCreationRequest) {
        ...
        if (isStatistics) lastSplitWaypoint = waypoint;
    ...
    private void restartTrack(Track track) {
        ...
        Waypoint waypoint = myTracksProviderUtils.getLastWaypoint(
                recordingTrackId, Waypoint.WaypointType.STATISTICS);
